...
export default function TxItem({ t }: { t: Transaction }) {
      ...
      <div className="col-span-2">
        <div className="flex items-center p-1">
          ...
          <div className="flex flex-col px-1 mx-1 dark:text-slate-400">
            {user.pubKey && user.pubKey === t.from ? (
              <div className="font-semibold text-orange-500 truncate w-28 hover:text-orange-300">Me</div>
            ) : (
            ...
            {t.timestamp && (
              <div className="text-xs dark:text-slate-500">{moment(new Date(t.timestamp)).fromNow()}</div>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="col-span-4">
        <div className="flex flex-col items-start justify-center p-1 text-slate-400">
          <div className="flex items-center justify-start">
            from:
            {user.pubKey && user.pubKey === t.from ? (
              <div className="inline-block ml-1 font-semibold text-orange-500 truncate w-28 hover:text-orange-300">
                Me
              </div>
            ) : (
